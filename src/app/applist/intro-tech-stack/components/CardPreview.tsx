...
const CardPreview = forwardRef<HTMLDivElement, CardPreviewProps>(
  ({ profile, techs, orientation = "vertical", onOrientationChange }, ref) => {
    // ソーシャルメディアのURL生成関数
    ...
    return (
      <div className={`w-full mx-auto ${isHorizontal ? "max-w-4xl" : "max-w-md"}`}>
        {/* 向き切り替えボタン */}
        {onOrientationChange && (
        ...
        >
          {/* ヘッダー部分 */}
          <div
            className={`hero bg-gradient-to-br from-primary to-secondary text-primary-content ${
              isHorizontal ? "rounded-l-2xl" : "rounded-t-2xl"
            } ${isHorizontal ? "w-80" : ""}`}
          >
            <div className={`hero-content ${isHorizontal ? "py-6 px-6" : "py-8"}`}>
              <div
                className={`flex items-center ${isHorizontal ? "flex-col space-y-4" : "space-x-6"}`}
              >
                {/* プロフィール画像 */}
          ...
          {/* 技術スタック部分 */}
          <div className={`card-body ${isHorizontal ? "flex-1 p-8" : "p-6"}`}>
            {techs.length > 0 ? (
          ...
          {/* フッター */}
          <div className={`card-actions justify-between ${isHorizontal ? "p-8 pt-0" : "p-6 pt-0"}`}>
